...
p <- web_visit %>% ggplot( aes(y=pages)) + geom_boxplot() + facet_grid(portal ~ version, scales = "free_x", space = "free_x") + theme( axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + labs(title="Boxplots of website visit data")
...
p <- web_visit %>% ggplot( aes(x=pages)) + geom_histogram(bins=15) + facet_grid(portal ~ version) + theme() + labs(title="Histogram of website visit data")
