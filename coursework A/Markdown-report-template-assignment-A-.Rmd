...
The most complex model tested uses the celebrity as a independent variable to predict the score. As we can expect the tweet sentiments to be normally distributed around 0 with almost all data between -5 and 5 due to the tweet word limit, a normally distributed prior with $\mu=0$ and $\sigma^2 =2$ is used.
...
Comparing the null model with the model using celebrity as a independent variable predictor, we can see that the latter model has a lower WAIC and thus a better fit, indicating that using celebrity as a predictor improves the fit of the model.
...
```
The plot clearly shows that there is a greater difference between the old and new versions of the website for the company portal. To Simple effect analysis will be conducted through contrasts on the consumer and company variables.
...
``` 
The WAIC analysis above indicates that the extensive model described at the beginning of the section has the best goodness of fit according to the WAIC comparison. The table below the WAIC comparisons shows the 95% credible intervals for all the model parameters. The only parameter that has null in its credible interval is the $a$ parameter which is the intercept. This indicates that it is insignificant. 
...
# model2 <- lme(score ~ 1 , random = ~session|Subject, data = exp_data, method="ML")

intervals(model0, 0.95)
intervals(model1, 0.95)
# intervals(model2, 0.95)

pander(anova(model0,model1), caption = "Adding session as a fixed effect.")
# pander(anova(model0,model2), caption = "Adding session as a fixed effect and session as a random effect.")
