...
pander(leveneTest(semFrame$score, semFrame$Candidate, center = median), caption = "Results of performing Levene's test on the three groups of tweets about the three celebrities.")
...
pander(res.aov, "Effect pre-post measuring condition on the tweet sentiments of the three celebrities.")
```
...
hist(semFrame$score, main="Sentiment values of all tweets.")
x <- seq(-5, 5, length=100)
y <- dnorm(x, mean=0, sd=2)
plot(x, y, type="l", lwd=1, main="Normal distribution with a mean of 0 and a standard deviation of 2.")
```

#### Model comparison

<!-- Conduct model analysis and provide brief interpretation of the results -->
As 0 is in none of the confidence intervals for $a_1, a_2,$ and $a_3$, we can conclude that the distribution of tweet sentiments for all three celebrities vary significantly from the prior we have given. Furthermore, comparing the null model with the model using celebrity as a independent variable predictor, we can see that the latter model has a lower WAIC and thus a better fit, indicating that using celebrity as a predictor improves the fit of the model.
...
$$\text{PageVisits} \sim N(\mu, \sigma) \\ \mu = a + b \cdot \text{Version} + c \cdot \text{Portal} + d\cdot \text{Version} \cdot \text{Portal} \\ a \sim \text{Gamma}(1, 2) \\b \sim N(0, 1) \\ c \sim N(0, 1) \\ d \sim N(0, 1) \\ \sigma \sim U(0.1, 2)$$
...
Write a small section for a scientific publication, in which you report the results of the analyses, and explain the conclusions that can be drawn.
-->
There was a significant relationship between session and score, $M = 116.8139$ (95% CI 112.70, 120.93), $p = 0$. The relationship between score and session showed significant variance in intercepts across subjects, $SD = 46.51$ (95% CI 43.67, 49.54), $p = 0$. 
...
$\sigma_\text{Subject} \sim \text{Cauchy}(0, 100)T(0, \inf)$
$a \sim N(0, 100)$
$b \sim N(0, 5)$
$\sigma \sim \text{Cauchy}(0,50)T(0, \inf)$

For the prior of the standard deviations $\sigma_\text{Subject}$ and $\sigma$ we use a half Cauchy distribution. We use a Cauchy distrubtion with a large scale value, since we do not have much prior knowledge or belief about the true parameters. With this large scale, the half Cauchy distribution has, unlike the normal distribution, fat tails. This makes the distribution over values away from the mean more uniform than in a normal distribution. For parameters $a$, $a_\text{Subject}$ and $b$ we chose to use a normal distribution as prior.
