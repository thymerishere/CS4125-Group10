...
library(pander)
```
```{r, fig.cap="Boxplot of the sentiment values of the tweets for each celebrity.", fig.align='center', out.width='50%'}
...
```
```{r, fig.cap="Bar plot for the tweet sentiment values for each celebrity.", fig.align='center', out.width='50%'}
plot(p)
```
...
The most complex model tested uses the celebrity as an independent variable to predict the score. As we can expect the tweet sentiments to be normally distributed around 0 with almost all data between -5 and 5 due to the tweet character limit (280 characters), a normally distributed prior with $\mu=0$ and $\sigma =2$ is used for $\mu$, and we allow $\sigma$ to take a value between 0.0001 and 5. To investigate the influence of Celebrity we add it as a factor and transform the intercept parameter into an index variable, giving us the following model:
...
```{r bayes_celeb_waic, fig.cap="Comparison of the WAIC score for each model.", fig.align='center', out.width='50%'}
plot(compare(m0, m1))
```

```{r bayes_celeb_params, fig.cap="Confidence intervals for the parameters of the model with celebrity added as a predictor.", fig.align='center', out.width='50%'}
...
```{r, echo=FALSE, fig.align='center', out.width="50%", fig.cap="The conceptual for the research question about modeling the amount of page visits."}
...
```{r, warning=FALSE, message=FALSE, fig.cap="Histograms of page visits on the website per independent variable.", fig.align='center', out.width='50%'}
...
```{r, warning=FALSE, message=FALSE, fig.cap="Boxplots of page visits on the website per independent variable", fig.align='center', out.width='50%'}
...
```{r, message=FALSE, warning=FALSE, fig.cap="Density plots fitted to data of page visits", fig.align='center', out.width='50%'}
...
```{r, fig.cap="Histogram of score.", fig.align='center', out.width='50%'}
...
compare(m0,m1,m2)
```
```{r fig.cap="Comparing three Bayesian models of increasing complexity.", fig.align='center', out.width='50%'}
...
```{r, fig.cap="Parameters of m2.", fig.align='center', out.width='50%'}
plot(m2)
```
