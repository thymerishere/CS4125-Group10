...
```{r homogeniety of variance}
library(car)
CG <- subset(d, modeltype == "Transfer Learning")
TG <- subset(d, modeltype != "Transfer Learning")
var.test(CG$score, TG$score)
leveneTest(d$score, d$modeltype)
```
...
```{r}
model0 <- lme(score ~ 1, random = ~1|TeD, data = d, method="ML")
model1 <- lme(score ~ TrD1 + TrD2 + TrD3 + TrD4 + TrD5 + TrD6 + TrD8, random = ~1|TeD, data = d, method="ML")
```
